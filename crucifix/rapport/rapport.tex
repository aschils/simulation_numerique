...
\begin{equation}
U_{\textsl{i},\textsl{j}} = w_j \exp (I r x_{\textsl{i}}) \;,
\label{eq:stabilite_expl_sol}
\end{equation}

où $I \equiv \sqrt{-1}$. Partons de l'Equation~\ref{eq:schema_num_sans_erreur}:
...
l'Equation~\ref{eq:stabilite_expl_sol} dans le schéma numérique implicite
(Equation~\ref{eq:schema_implicite}). On obtient alors:
...
$\lambda_d = \lambda_a/2$, $\lambda_b = -1$ et $\lambda_a < 1$.
