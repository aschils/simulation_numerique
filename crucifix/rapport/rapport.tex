...
%selon (c'est à dire des nombres réels dans leur représentation
...
L'opérateur linéaire tangent au premier ordre de la norme d'une perturbation $\delta x$ est:

\begin{equation}\label{eq}
\frac{d \left\|\delta x \right\|}{d t} = J(x,t) \left\|\delta x \right\| \;.
\end{equation}
...
A partir de l'équation \eqref{eq}, on a:

\begin{equation}
 \ln(\left\|\delta x \right\|) = -kt + \text{Constante}\;.
\end{equation}

On utilise la condition initiale $\delta x_{0}$, ce qui donne :

\begin{equation}
 \ln(\left\|\delta x \right\|) = -kt + \ln(\left\|\delta x_{0} \right\|)\;.\label{pertu}
\end{equation}

On a donc:

\begin{equation}
\lambda_{max}= \lim_{t\rightarrow_{\infty}}\frac{1}{t} \ln(\left\|\delta x \right\|) = \lim_{t\rightarrow_{\infty}}\left(\frac{-kt+\ln(\left\|\delta x_{0} \right\|)}{t}\right)= -k\;.
\end{equation}

 
En outre, le système est chaotique si $\lambda_{max} > 0$. Pour la décroissance radioactive, cela signifie que $k<0$. Dans ce cas, l'équation \eqref{pertu} montre alors qu'une perturbation appliquée au système va croître de manière exponentiel avec le temps.
...
En utilisant fonction implémentée dans le fichier \code{exercice5.m}
on obtient $\lambda_{max} = -0.9205$ pour $\rho=5$,
$\lambda_{max} = -0.5766$ pour $\rho=10$, $\lambda_{max} = 0.9194$ pour $\rho=28$
et $\lambda_{max} = 1.2364$ pour $\rho=45$.

Un système est chaotique si $\lambda_{max} > 0$. Dès lors on voit
que les cas chaotiques sont $\rho=28$ et $\rho=45$. 
