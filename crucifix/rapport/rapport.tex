...
le premier bit correspond à l'exposant $0$ de $2$, le deuxième à l'exposant $-1$
de 2 et ainsi de suite.
Dès lors, pour la représentation d'un réel sur 16 bits on a:

\begin{equation}
x_i = \sum_{i=0}^{i=15} a_i 2^{-i}
\end{equation}

où les coefficients $a_i$ peuvent prendre la valeur $0$ ou $1$. Si on
applique à cette définition la formule de récurrence~\ref{eq:chaos_rec}
on obtient, si $x_i \le 0.5$:

\begin{equation}
x_{i+1} = 2 \sum_{i=0}^{i=15} a_i 2^{-i} = \sum_{i=0}^{i=15} a_i 2^{-(i-1)} = \sum_{i=0}^{i=14} a_{i+1} 2^{-i}\;.
\end{equation}

On voit donc que le nombre binaire a été shifté vers la gauche (une multiplication
par deux en binaire est en effet équivalent à un shift du nombre binaire vers
...
s'ajoute à la droite du nombre binaire. Si $x_i > 0.5$ on obtient:

\begin{equation}
x_{i+1} = 2-\sum_{i=0}^{i=14} a_{i+1} 2^{-i}\;.
\end{equation}
